...
if ( ! class_exists( 'CFU_Admin_Feedback' ) ) :

	/**
	 * The feedback.
	 */
	class CFU_Admin_Feedback {
		...
		public function __construct( $args ) {
			...
			// Add actions.
			add_action( 'admin_init', array( $this, 'cfu_check_installation_date' ) );
			add_action( 'admin_init', array( $this, 'cfu_set_no_bug' ), 5 );
		}
		 ...
		 */
		public function cfu_seconds_to_words( $seconds ) {
		 ...
		 */
		public function cfu_check_installation_date() {
			if ( ! get_site_option( $this->nobug_option ) || false === get_site_option( $this->nobug_option ) ) {
				...
				if ( ( time() - $install_date ) > $this->time_limit ) {
					add_action( 'admin_notices', array( $this, 'cfu_display_admin_notice' ) );
				}
			}
		}
		 ...
		 */
		public function cfu_display_admin_notice() {
			$screen = get_current_screen();

			if ( isset( $screen->base ) && 'plugins' === $screen->base ) {
				$no_bug_url = wp_nonce_url( admin_url( '?' . $this->nobug_option . '=true' ), 'cfup-feedback-nounce' );
				$time       = $this->cfu_seconds_to_words( time() - get_site_option( $this->date_option ) );
				?>
...
@media (max-width: 767px) {
	...
	.notice.cfup-notice .cfup-notice-inner .no-thanks {
			...
			<div class="notice updated cfup-notice">
				<div class="cfup-notice-inner">
					<div class="cfup-notice-icon">
						<img src="<?php echo esc_url( CUSTOM_FONT_UPLOADER_PLUGIN_URL . '/admin/assets/images/custom_font_uploder.png' ); ?>" alt="<?php echo esc_html__( 'Custom Font Uploader', 'cfup' ); ?>"/>
					</div>
		 ...
		 */
		public function cfu_set_no_bug() {

			// Bail out if not on correct page.
			if ( ! isset( $_GET['_wpnonce'] ) || ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'cfup-feedback-nounce' ) || ! is_admin() || ! isset( $_GET[ $this->nobug_option ] ) || ! current_user_can( 'manage_options' ) ) ) {
				return;
			}
...
/*
* Instantiate the CFU_Admin_Feedback class.
*/
new CFU_Admin_Feedback(
	array(
